...
def get_args():
    ...
    ap.add_argument(
        "-c", "--categories", help="page categories (only post)", default=[], nargs="*"
    )
...
class TemplateCreator:
    ...
    def _init_page_tmp(self):
        return (
            os.path.join(PJTPATH, self.norm_path, "index.md"),
            self.tmp_str.format(
                self.title,
                self._get_date(),
                self.title,
                self._get_permalink(self.norm_path),
            ),
        )
    ...
    def _init_post_tmp(self):
        ...
        return (
            os.path.join(PJTPATH, "_posts", self.norm_path, f"{date}-{file_name}.md"),
            self.tmp_str.format(
                self.title,
                self._get_date(),
                str(self.categories),
                self._get_permalink(
                    self.norm_path, f"{datetime.date.today():%Y/%m/%d}", file_name
                ).strip("/"),
            ),
        )
